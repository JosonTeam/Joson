              ...
              andIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section)
    {
        ...
        case 0:
        {
            ...
            [listButton setImage : [UIImage imageNamed:@"toolbar_favorite@2x.png"]
                        forState : UIControlStateNormal];
...
//                forControlEvents : UIControlEventTouchUpInside];
            ...
            if ([arr containsObject:@"retweeted_status"])
            {
                ...
                //转发的微博的底层view
                UIView * content_View1 = [[UIView alloc] initWithFrame : CGRectMake(0, text.frame.origin.y + 5 + CGRectGetHeight(text.frame), _width, [Factory contentHeight:_weiboSource[@"retweeted_status"][@"text"] width:_width-10] + ceil(content_Pic1.count/3.0) * _width/3 + 40)];
                
                content_View.frame = CGRectMake(0, 35, _width, [Factory contentHeight:_weiboSource[@"text"] width:_width-10] + [Factory contentHeight:_weiboSource[@"retweeted_status"][@"text"] width:_width-10] + ceil(content_Pic1.count/3.0) * 90);
                
                if (content_Pic1.count == 1)
                {
                    
                    content_View1.frame = CGRectMake(0, text.frame.origin.y + 5 + CGRectGetHeight(text.frame), _width, [Factory contentHeight:_weiboSource[@"retweeted_status"][@"text"] width:_width-10] + _width + 30);
                ...
                for (int i = 0; i < content_Pic1.count; i++)
                {
                    ...
                    NSMutableArray * button_Name = [[NSMutableArray alloc]initWithArray:@[@"转发" , @"评论" , @"赞"]];
                    
                    if ([_weiboSource[@"retweeted_status"][@"reposts_count"] intValue] > 0)
                    {
                        
                        [button_Name replaceObjectAtIndex:0
                                              withObject : [NSString stringWithFormat:@"%d",[_weiboSource[@"retweeted_status"][@"reposts_count"] intValue]]];
                        
                    }
                    
                    if ([_weiboSource[@"retweeted_status"][@"comments_count"] intValue] > 0)
                    {
                        
                        [button_Name replaceObjectAtIndex:1
                                              withObject : [NSString stringWithFormat:@"%d",[_weiboSource[@"retweeted_status"][@"comments_count"] intValue]]];
                        
                    }
                    
                    if ([_weiboSource[@"retweeted_status"][@"attitudes_count"] intValue] > 0)
                    {
                        
                        [button_Name replaceObjectAtIndex:2
                                              withObject : [NSString stringWithFormat:@"%d",[_weiboSource[@"retweeted_status"][@"attitudes_count"] intValue]]];
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        UIButton * button = [UIButton buttonWithType : UIButtonTypeCustom];
                        button.frame = CGRectMake(_width/2+i*(_width/2)/3, CGRectGetHeight(content_View1.frame) - 30, (_width/2)/3-1, 25);
                        ...
                        button.selected = YES;
                        [content_View1 addSubview:button];
            ...
            else
            {
                ...
                for (int i = 0; i < content_Pic.count; i++)
                {
                    ...
                    [_weiboContentPic addObject:[UIImage imageNamed:@"message_placeholder_picture@2x.png"]];
                    UIImageView * image = [[UIImageView alloc]initWithFrame:CGRectMake(5 + (i%3)*_width/3, text.frame.origin.y + 5+ CGRectGetHeight(text.frame) + (i/3)*_width/3, _width/3-10, _width/3-10)];

                    if (content_Pic.count == 4)
                    {
                        image.frame = CGRectMake(5 + (i%2)*_width/3, text.frame.origin.y + 5 + CGRectGetHeight(text.frame) + (i/2)*_width/3, _width/3-10, _width/3-10);
                    }
                    ...
                    if ([_weiboContentPic[i] isEqual : [UIImage imageNamed:@"message_placeholder_picture@2x.png"]])
                    {
                        ...
                        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                            ...
                            NSData * data = [[NSData alloc] initWithContentsOfURL:[NSURL URLWithString:_weiboSource[@"pic_urls"][i][@"thumbnail_pic"]]];
        ...
        case 1:
        {
            ...
            NSDictionary * dic = [MicroBlogOperateForSina getAnswerListOfWeiboWithAccessToken:_access_token andId:JDZ([_weiboSource[@"id"] integerValue])];
            NSDictionary * dic1 = [MicroBlogOperateForSina getReportListOfWeiboWithAccessToken:_access_token andId:JDZ([_weiboSource[@"id"] integerValue])];
            ...
            case 0:
            {
                ...
                if (_weiboSource[@"retweeted_status"])
                {
                    ...
                    if (content_Pic1.count == 1)
                    {
                        ...
                        return [Factory contentHeight:_weiboSource[@"text"] width:_width-10] + [Factory contentHeight:_weiboSource[@"retweeted_status"][@"text"] width:_width-10] + 80+_width;

                    }
                    
                    return [Factory contentHeight:_weiboSource[@"text"] width:_width-10] + [Factory contentHeight:_weiboSource[@"retweeted_status"][@"text"] width:_width-10] + ceil(content_Pic1.count/3.0) * _width/3 + 90;
                ...
                else
                {
                    ...
                    return ceil(content_Pic.count/3.0) * _width/3 + [Factory contentHeight:_weiboSource[@"text"] width:_width-10] + 50;
                }
...
- (void)click_Button:(UIButton *)sender
{
    ...
    else
    {
        ...
        [MicroBlogOperateForSina destroyFollowingWithAcccessToken:_access_token
                                                               iD:JDZ([_source[sender.tag-1][@"id"] integerValue])];
